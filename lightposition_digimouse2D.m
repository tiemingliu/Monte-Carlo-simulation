...
vmcmedium.absorption_coefficient(disc_skull) = 0.0136; 
vmcmedium.scattering_anisotropy(disc_skull) = caculate_us_skull(wavelength);           % scattering anisotropy parameter [unitless]
...
% vmcmedium.gruneisen_parameter(disc_blood_indices) = 0.9;
...
%% Calculate the distance and width of the light source
...
light_width = (lightend1_column - lightbegin1_column)*0.1 % [mm]
light_distance = (lightend1_column - lightbegin2_column)*0.1 - light_width %[mm]
...
%% calculate the Coefficient of Variation COV (inside the brain)
 ...
 COV = zeros(20,1);
 SUM_absorbedenergy = zeros(20,1);
 i=1;
 for angle = pi/7:pi/180:pi/4

    filename = ['absorbenergy' '_' num2str(angle) '.mat'];
    load(['/Users/liutieming/Documents/MATLAB/ValoMC/result_angle/',filename],'absorbed_energy');
    norm_data = absorbed_energy;
    mean_absorbedenergy = mean(norm_data(disc_braintissue),"all");
    std_absorbedenergy = std2(norm_data(disc_braintissue));
    
    COV(i,1) = std_absorbedenergy/mean_absorbedenergy;
    SUM_absorbedenergy(i,1) = sum(norm_data(disc_braintissue));
    i = i+1;
 end
figure(1)
angle = 180/7:180/180:180/4;
plot(round(90-angle,0),(1-COV)*100,'-o','LineWidth',2,'Color','k');
...
figure(2)
plot(round(90-angle,0),SUM_absorbedenergy,'-o','LineWidth',2,'Color','b');
xtickformat("degrees")
xlabel('Angle [degree]');
ylabel('Total absorbed energy in the brain [a.u.]');
set(gca,'FontSize',18);
grid on
